    ...
    <div style=" padding-bottom:60px">
        ...
        <div class="training_direction">
            ...
            <div class="training_direction_item">
                课程安排
                <p>
                    <span style="color:#000">{{details.start_date}}</span>&nbsp;&nbsp;至&nbsp;&nbsp;
                    <span style="color:#000">{{details.end_date }}</span>
                </p>
            </div>
            <div class="training_direction_item">
                报名人数（已报名
                <span style="color:#000">{{details.users_count}}</span> 人）
                <p>
            ...
            <div class="training_direction_item">
                ...
                <el-table :data="ss" style="width: 100%">
                    <el-table-column prop="time" label="" width="43">
                    </el-table-column>
                    <el-table-column label="周一" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week1">
                        </template>
                    </el-table-column>
                    <el-table-column label="周二" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week2">
                        </template>
                    </el-table-column>
                    <el-table-column label="周三" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week3">
                        </template>
                    </el-table-column>
                    <el-table-column label="周四" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week4">
                        </template>
                    </el-table-column>
                    <el-table-column label="周五" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week5">
                        </template>
                    </el-table-column>
                    <el-table-column label="周六" width="43">
                        <template scope="scope">
                            <img :src="scope.row.week6">
                        </template>
                    </el-table-column>
                    <el-table-column label="周日" width="43">
                        <template scope="scope">
                ...
                <div class="classes">
                    ...
                    <ul class="classes-list">
                        <li v-for="(item, index) in p" :class="{checked:item.isC}" :key="index" @click='checkIt(item, index)'>
                            ...
                            <s style="font-size:12px">原价{{item.old_price*0.01}}元</s>&nbsp;&nbsp;
                            <span>{{item.price == 0?'免费':'¥ '+item.price*0.01 }}</span>
                        </li>
                    </ul>
                    <div class="check-num">
                        <span>选择数量</span>
                        <span>(剩余{{details.capacity_max - details.users_count}}个名额)</span>
                        <div class="min" @click="peopleNum<2?1:peopleNum--">-</div>
...
export default {
    ...
    computed: {
        ...
        p() {
            var arr = []
            this.details.prices.map((x, i) => {
                i == 0 ? this.$set(x, 'isC', true) : this.$set(x, 'isC', false)
                arr.push(x)
            })
            return arr
        }
    ...
    methods: {
        ...
        getDetails() {
            ...
            axios.get('/api/trains/' + this.$route.params.id + '/detail')
                .then(function(response) {
                    ...
                    self.trains_price_id = self.details.prices[0].id
        ...
        enroll() {
            if (!this.trains_price_id) {
                this.$vux.toast.text('请选择课程！', 'bottom')
    ...
    watch: {
        '$route': 'getDetails',
        'peopleNum': function(val, oldval) {
            let se = this.details.prices.filter((el) => {
                return el.isC
            })[0]
            this.details.price = val * se.price
        }
    }
}
</script>
...
.classes-list li.checked span:nth-child(1),
li.checked span:nth-child(2) {
    color: #FF6868;
}
</style>
<style>
